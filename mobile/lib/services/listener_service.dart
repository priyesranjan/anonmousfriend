...
class ListenerService {
  ...
  Future<ListenerResult> getListeners({
    ...
    if (isBusy != null) queryParams['is_busy'] = isBusy.toString();
  ...
  Future<ListenerResult> getSmartMatchListeners({
    ...
    // Always pass is_verified_only so the backend knows which listener_type to filter on.
    // true → listener_type='full' (professional expert)
    // false → listener_type='casual' (regular listener)
    queryParams['is_verified_only'] = (isVerifiedOnly == true) ? 'true' : 'false';
