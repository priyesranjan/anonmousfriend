...
router.post('/', authenticateAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [amount, extra_percent_or_amount || 0, badge_text || null, is_active !== false, sort_order || 0, is_unlimited || false, unlimited_days || 0, is_first_time_only || false]
    );
...
router.put('/:id', authenticateAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [amount, extra_percent_or_amount, badge_text, is_active, sort_order, is_unlimited, unlimited_days, is_first_time_only, id]
    );
